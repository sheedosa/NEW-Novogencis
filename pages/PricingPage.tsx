...
const PricingSection = ({ title, items, icon, isPackages = false, onNavigate }: { title: string, items: PriceItem[], icon: string, isPackages?: boolean, onNavigate: (page: Page) => void }) => (
  <div className="mb-20">
    <div className="flex items-center gap-4 mb-10">
      <div className="w-12 h-12 bg-primary/10 rounded-2xl flex items-center justify-center text-primary">
        <span className="material-symbols-outlined text-2xl">{icon}</span>
      </div>
      <h2 className="text-2xl md:text-3xl font-black text-text-main tracking-tight uppercase tracking-widest">{title}</h2>
    </div>
    <div className={`grid grid-cols-1 ${isPackages ? 'lg:grid-cols-3' : 'md:grid-cols-2'} gap-6`}>
      {items.map((item, idx) => (
        ...
        >
          {item.isPopular && (
            <div className="absolute top-0 right-0 bg-primary text-clinical-dark text-[10px] font-black px-6 py-2 rounded-bl-2xl uppercase tracking-widest flex items-center gap-1">
              <span className="material-symbols-outlined text-[12px]">star</span> Most Popular
            </div>
          )}
          <div>
            <h3 className="text-xl font-black text-text-main leading-tight mb-2 pr-20 group-hover:text-primary transition-colors">
              {item.title}
            </h3>
            {item.subtitle && (
              <p className="text-text-muted text-xs font-bold uppercase tracking-widest mb-6 leading-relaxed">
                ({item.subtitle})
              </p>
            )}
            {item.features && (
              <ul className="space-y-3 mb-8">
                {item.features.map((feature, fIdx) => (
                  <li key={fIdx} className="flex items-start gap-3 text-sm text-text-muted font-medium">
          ...
          <div className="mt-auto flex items-end justify-between border-t border-gray-50 pt-6">
            <div className="flex flex-col">
              <span className="text-xs font-black text-text-muted uppercase tracking-widest mb-1">Investment</span>
              <span className="text-3xl md:text-4xl font-black text-text-main">{item.price}</span>
            </div>
            <button 
...
const PricingPage: React.FC<PricingPageProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="animate-fade-in bg-bg-soft min-h-screen">
      {/* Hero Section */}
      <section className="relative px-6 md:px-20 pt-32 md:pt-44 pb-20 md:pb-32 overflow-hidden bg-white">
        ...
        <div className="max-w-[1200px] mx-auto relative z-10 text-center">
          ...
          <h1 className="text-4xl md:text-7xl font-black text-text-main tracking-tight leading-[1.1] mb-8 animate-fade-up">
            Treatments & <span className="text-primary italic font-serif">Packages</span>
          </h1>
          <p className="text-text-muted text-lg md:text-xl max-w-2xl mx-auto font-medium animate-fade-up delay-100 mb-4">
            Transparent pricing for world-class regenerative care.
          </p>
          <p className="text-primary font-black text-[10px] md:text-xs uppercase tracking-[0.2em] animate-fade-up delay-150">
          ...
          <div className="mt-12 flex flex-col items-center gap-6 animate-fade-up delay-200">
            <button 
              onClick={() => onNavigate(Page.Assessment)}
              className="bg-primary text-clinical-dark px-10 py-4 rounded-full font-black text-sm uppercase tracking-widest hover:bg-clinical-dark hover:text-white transition-all shadow-xl shadow-primary/20 hover:scale-105 active:scale-95"
            >
            ...
            <div className="flex flex-col items-center gap-2 mt-4">
               <span className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.3em] text-text-muted/60">Payment plans available via</span>
      ...
      <section className="px-6 md:px-20 py-20 md:py-32">
        <div className="max-w-[1440px] mx-auto">
          ...
          <PricingSection title="Restoration Packages" items={packages} icon="calendar_month" isPackages={true} onNavigate={onNavigate} />
          ...
          <div className="mt-20 max-w-[1200px] mx-auto bg-clinical-dark rounded-[2.5rem] md:rounded-[3rem] p-8 md:p-20 relative overflow-hidden shadow-2xl transform translate-z-0">
            ...
            <div className="relative z-10 flex flex-col lg:flex-row items-center gap-12 md:gap-16">
              <div className="w-full lg:w-3/5 text-center lg:text-left">
                <span className="text-primary font-black uppercase tracking-[0.4em] text-[10px] mb-6 block">START YOUR JOURNEY</span>
                ...
                <button 
                  onClick={() => onNavigate(Page.Assessment)}
                  className="bg-primary text-clinical-dark px-8 py-3.5 rounded-full font-bold text-sm uppercase tracking-widest hover:bg-white hover:scale-105 active:scale-95 transition-all duration-300 shadow-lg shadow-primary/10"
                >
